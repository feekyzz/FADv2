...
class CARAFEFunction(Function):
    ...
    def forward(ctx, features: Tensor, masks: Tensor, kernel_size: int,
        ...
        if torch.cuda.is_available():
            ...
            rfeatures = rfeatures.to(device)
            rmasks = rmasks.to(device)
            routput = routput.to(device)
        ...
        if features.requires_grad or masks.requires_grad or torch.__version__ == 'parrots':
            ctx.save_for_backward(features, masks, rfeatures)

        # 返回时需要将 output 保持在与输入张量一致的设备
        return output
